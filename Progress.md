...
| F8 | Pedidos + pagos (saga checkout con pago simulado) | DONE |
...
## Fase 9 — UX + admin (identidad visual + panel admin)

- [x] Migración TSX completa de páginas (Login, Register, Account, Checkout, Home, Products, ProductDetail, Cart, Orders, ForgotPassword, ResetPassword, AdminDashboard)
- [x] Identidad visual: variables CSS con colores ElectroShop, tipografía, componentes UI consistentes
- [x] Admin dashboard con métricas básicas

**Criterios de aceptación:** build TS OK, lint 0 errores, tests frontend PASS (25/25→57/57).

## Fase 10 — Seguridad (hardening + security tests)

- [x] Security hardening: Helmet, CORS estricto, rate-limit, JWT edge verification
- [x] Auth-service security suite: 35 tests (A1–A12 matrices: login failures, token validation, password policy, rate-limit, timing)
- [x] RolesGuard specs en todos los servicios (B1–B8)
- [x] DTO validation + whitelist en todos los servicios (C1–C6)
- [x] Inventory business rules tests (D1–D7)
- [x] Gateway network tests (E1–E6)
- [x] npm audit clean, gitleaks CI, trivy fs scan
- [x] CI jobs: security-tests, trivy fs

**Criterios de aceptación:** security suite 35/35 PASS, npm audit 0 vulnerabilities, gitleaks 0 findings.

## Fase 11 — Testing + CI (gates obligatorios, e2e)

- [x] Coverage thresholds in all 8 service package.json (critical: 75%, normal: 70%)
- [x] Frontend vitest.config.ts: 60% stmts/functions/lines, 55% branches (scope: api/context/hooks)
- [x] Coverage enforcement: `npm run test:cov` with `--coverage --coverageThreshold` as mandatory CI gate
- [x] CI matrix fix: `matrix.package.dir` → `matrix.dir` (quality job was broken)
- [x] Frontend tests enforced in CI (`has-test: false` removed)
- [x] Auth-service now `strict: true` in CI (debt resolved)
- [x] Order-service: http-exception.filter.spec (6 tests) + jwt.strategy.spec (2 tests) → 85.4% coverage
- [x] Payment-service: http-exception.filter.spec (6) + jwt.strategy.spec (2) + roles.guard.spec (5) → 84.1% coverage
- [x] Gateway: proxy.controller.spec (13) + auth-proxy.controller.spec (4) + request-id.middleware.spec (3) + http-exception.filter.spec (6) + health.service.spec (4) → 81.3% coverage
- [x] Cart-service: http-exception.filter.spec (6) + roles.guard.spec (2) + jwt.strategy.spec (2) → 84.3% coverage
- [x] Inventory-service: inventory.service.spec (14) + inventory.controller.spec (6) + http-exception.filter.spec (6) + roles.guard.spec (2) + jwt.strategy.spec (2) → 83.1% coverage
- [x] Security-tests CI job: auth-service test:security (35 tests)
- [x] Trivy fs scan CI job: CRITICAL+HIGH = fail, ignore unfixed
- [x] E2E Playwright setup: e2e/ package, critical-flow.spec.ts (register→login→catalog→cart→checkout→order)
- [x] CI e2e job: compose up → wait healthy → seed → playwright → upload report/traces
- [x] docs/TESTING.md: strategy, thresholds, CI gates, security catalog, running tests, gaps

**Criterios de aceptación:**

- All 8 backend services pass coverage thresholds (critical ≥75%, normal ≥70%)
- Frontend passes 60%/55% thresholds (api/context/hooks scope)
- Security suite 35/35 PASS
- npm audit 0 vulnerabilities, gitleaks 0, trivy 0 CRITICAL
- CI quality matrix passes for all 8 packages + frontend
- E2E critical flow test defined and wired in CI
