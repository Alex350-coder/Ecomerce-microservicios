...
describe('HttpExceptionFilter', () => {
  ...
  it('propagates request id header', () => {
    const json = jest.fn();
    const status = jest.fn().mockReturnValue({ json });
    filter.catch(new Error('boom'), mockHost(json, status, { 'x-request-id': 'rid-cart' }));
    expect(json).toHaveBeenCalledWith(expect.objectContaining({ requestId: 'rid-cart' }));
  });
  ...
  it('extracts error field from HttpException object', () => {
    const json = jest.fn();
    const status = jest.fn().mockReturnValue({ json });
    filter.catch(
