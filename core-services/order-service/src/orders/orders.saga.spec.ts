...
describe('OrdersService - Saga execution', () => {
    ...
    global.fetch = jest
      ...
      .mockResolvedValueOnce({
        ...
        json: () =>
          Promise.resolve({
            reservationId: 'r1',
            items: [{ productId: 'p1', quantity: 2, available: true }],
          }),
      })
      .mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ id: 'pay-1', status: 'approved' }),
      })
  ...
  it('should mark order as FAILED when payment is declined', async () => {
    ...
    global.fetch = jest
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ id: 'pay-1', status: 'declined' }),
      })
      .mockResolvedValueOnce({ ok: true }) as any;
  ...
  it('should mark order as FAILED when stock reservation fails', async () => {
    ...
    global.fetch = jest.fn().mockResolvedValueOnce({
      ...
      json: () =>
        Promise.resolve({
          reservationId: 'r1',
          items: [{ productId: 'p1', quantity: 2, available: false }],
        }),
    }) as any;
